...
void GlobalMySQLMetadataDB::open() {
    ...
    fmt::format_to(
            ...
            m_table_prefix,
            streaming_archive::cMetadataDB::ArchivesTableName,
            get_set_field_sql(update_archive_size_stmt_field_names,
                              0,
                              enum_to_underlying_type(UpdateArchiveSizeStmtFieldIndexes::Length)),
            streaming_archive::cMetadataDB::Archive::Id);
    SPDLOG_DEBUG("{:.{}}", statement_buffer.data(), statement_buffer.size());
    ...
    fmt::format_to(statement_buffer_ix,
                   ...
                   get_set_field_sql(file_field_names,
...
void GlobalMySQLMetadataDB::add_archive(string const& id,
    ...
    statement_bindings.bind_varchar(enum_to_underlying_type(ArchivesTableFieldIndexes::Id),
                                    id.c_str(),
                                    id.length());
    ...
    statement_bindings.bind_int64(enum_to_underlying_type(ArchivesTableFieldIndexes::EndTimestamp),
                                  end_timestamp);
    ...
    statement_bindings.bind_uint64(enum_to_underlying_type(ArchivesTableFieldIndexes::Size),
                                   compressed_size);
    auto const& creator_id = metadata.get_creator_id();
    statement_bindings.bind_varchar(enum_to_underlying_type(ArchivesTableFieldIndexes::CreatorId),
                                    creator_id.c_str(),
                                    creator_id.length());
    auto creation_num = metadata.get_creation_idx();
    statement_bindings.bind_uint64(enum_to_underlying_type(ArchivesTableFieldIndexes::CreationIx),
                                   creation_num);
...
void GlobalMySQLMetadataDB::update_archive_metadata(
    ...
    statement_bindings.bind_uint64(
            ...
            uncompressed_size);
    auto compressed_size = metadata.get_compressed_size_bytes();
    statement_bindings.bind_uint64(enum_to_underlying_type(UpdateArchiveSizeStmtFieldIndexes::Size),
                                   compressed_size);
...
void GlobalMySQLMetadataDB::update_metadata_for_files(
    ...
    for (auto file : files) {
        ...
        statement_bindings.bind_varchar(enum_to_underlying_type(FilesTableFieldIndexes::Id),
                                        id_as_string.c_str(),
                                        id_as_string.length());

        auto const orig_file_id_as_string = file->get_orig_file_id_as_string();
        statement_bindings.bind_varchar(enum_to_underlying_type(FilesTableFieldIndexes::OrigFileId),
                                        orig_file_id_as_string.c_str(),
                                        orig_file_id_as_string.length());

        auto const& orig_path = file->get_orig_path();
        statement_bindings.bind_varchar(enum_to_underlying_type(FilesTableFieldIndexes::Path),
                                        orig_path.c_str(),
                                        orig_path.length());
        ...
        statement_bindings.bind_int64(enum_to_underlying_type(FilesTableFieldIndexes::EndTimestamp),
                                      end_ts);
        ...
        statement_bindings.bind_uint64(enum_to_underlying_type(FilesTableFieldIndexes::NumMessages),
                                       num_messages);

        statement_bindings.bind_varchar(enum_to_underlying_type(FilesTableFieldIndexes::ArchiveId),
                                        archive_id.c_str(),
                                        archive_id.length());
...
bool GlobalMySQLMetadataDB::get_file_split(string const& orig_file_id,
                                           size_t message_ix,
                                           string& archive_id,
