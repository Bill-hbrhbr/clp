...
namespace clp {
namespace {
void create_tables(vector<pair<string, string>> const& archive_field_names_and_types,
...
SQLitePreparedStatement get_archives_select_statement(SQLiteDB& db) {
    auto statement_string = fmt::format("SELECT {} FROM {} ORDER BY {} ASC, {} ASC",
...
SQLitePreparedStatement get_archives_for_time_window_select_statement(SQLiteDB& db,
                                                                      epochtime_t begin_ts,
                                                                      epochtime_t end_ts) {
    auto statement_string
            = fmt::format("SELECT {} FROM {} WHERE {} <= ? AND {} >= ? ORDER BY {} ASC, {} ASC",
                          streaming_archive::cMetadataDB::Archive::Id,
                          streaming_archive::cMetadataDB::ArchivesTableName,
...
SQLitePreparedStatement get_archives_for_file_select_statement(SQLiteDB& db,
...
SQLitePreparedStatement get_file_split_statement(SQLiteDB& db,
                                                 string const& orig_file_id,
                                                 size_t message_ix) {
    auto statement_string
            = fmt::format("SELECT DISTINCT {}.{}, {}.{} FROM {} JOIN {} ON {}.{} = {}.{} "
                          "WHERE {}.{} = ?1 AND ?2 >= {}.{} AND ?2 < ({}.{} + {}.{}) "
                          "ORDER BY {} ASC, {} ASC",
                          streaming_archive::cMetadataDB::ArchivesTableName,
                          streaming_archive::cMetadataDB::Archive::Id,
...
void GlobalSQLiteMetadataDB::open() {
    ...
    archive_field_names_and_types[enum_to_underlying_type(
                                          ArchivesTableFieldIndexes::BeginTimestamp)]
            .first = streaming_archive::cMetadataDB::Archive::BeginTimestamp;
    archive_field_names_and_types[enum_to_underlying_type(
                                          ArchivesTableFieldIndexes::BeginTimestamp)]
            .second = "INTEGER";

    archive_field_names_and_types[enum_to_underlying_type(ArchivesTableFieldIndexes::EndTimestamp)]
            .first = streaming_archive::cMetadataDB::Archive::EndTimestamp;
    archive_field_names_and_types[enum_to_underlying_type(ArchivesTableFieldIndexes::EndTimestamp)]
            .second = "INTEGER";

    archive_field_names_and_types[enum_to_underlying_type(
                                          ArchivesTableFieldIndexes::UncompressedSize)]
            .first = streaming_archive::cMetadataDB::Archive::UncompressedSize;
    archive_field_names_and_types[enum_to_underlying_type(
                                          ArchivesTableFieldIndexes::UncompressedSize)]
            .second = "INTEGER";
    ...
    archive_field_names_and_types[enum_to_underlying_type(ArchivesTableFieldIndexes::CreatorId)]
            .first = streaming_archive::cMetadataDB::Archive::CreatorId;
    archive_field_names_and_types[enum_to_underlying_type(ArchivesTableFieldIndexes::CreatorId)]
            .second = "TEXT";

    archive_field_names_and_types[enum_to_underlying_type(ArchivesTableFieldIndexes::CreationIx)]
            .first = streaming_archive::cMetadataDB::Archive::CreationIx;
    archive_field_names_and_types[enum_to_underlying_type(ArchivesTableFieldIndexes::CreationIx)]
            .second = "INTEGER";
...
void GlobalSQLiteMetadataDB::add_archive(string const& id,
                                         streaming_archive::ArchiveMetadata const& metadata) {
    if (false == m_is_open) {
        throw OperationFailed(ErrorCode_NotInit, __FILENAME__, __LINE__);
    }

    m_insert_archive_statement->bind_text(enum_to_underlying_type(ArchivesTableFieldIndexes::Id)
                                                  + 1,
                                          id,
                                          false);
    ...
    m_insert_archive_statement->bind_int64(enum_to_underlying_type(ArchivesTableFieldIndexes::Size)
                                                   + 1,
...
bool GlobalSQLiteMetadataDB::get_file_split(string const& orig_file_id,
                                            size_t msg_ix,
                                            string& archive_id,
