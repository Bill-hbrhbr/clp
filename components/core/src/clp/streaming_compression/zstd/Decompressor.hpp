...
public:
    ...
    auto open(FileReader& file_reader, size_t file_read_buffer_capacity) -> void override;
     ...
     * @return ErrorCode_Success on success
     */
    auto open(std::string const& compressed_file_path) -> ErrorCode;
...
private:
    ...
    FileReader* m_file_reader{nullptr};
    size_t m_file_reader_initial_pos{0};
    std::unique_ptr<char[]> m_file_read_buffer;
    size_t m_file_read_buffer_length{0};
    size_t m_file_read_buffer_capacity{0};

    ZSTD_inBuffer m_compressed_stream_block;

    size_t m_decompressed_stream_pos{0};
    size_t m_unused_decompressed_stream_block_size{0};
