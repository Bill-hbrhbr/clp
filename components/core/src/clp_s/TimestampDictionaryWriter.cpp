...
void TimestampDictionaryWriter::write(std::stringstream& stream) {
    ...
    write_numeric_value<uint64_t>(stream,
                                  m_string_pattern_and_id_pairs.size()
                                          + m_numeric_pattern_to_id.size());
...
auto TimestampDictionaryWriter::ingest_string_timestamp(std::string_view key,
                                                        int32_t node_id,
    ...
    for (auto const& [quoted_pattern, pattern_id] : m_string_pattern_and_id_pairs) {
        auto const parsing_result{timestamp_parser::parse_timestamp(timestamp,
                                                                    quoted_pattern,
                                                                    is_json_literal,
                                                                    m_generated_pattern)};
    ...
    auto const parsing_result{
            timestamp_parser::search_known_timestamp_patterns(timestamp,
    ...
    m_string_pattern_and_id_pairs.emplace_back(std::move(quoted_pattern_result.value()),
                                               new_pattern_id);
    return {epoch_timestamp, new_pattern_id};
}

auto TimestampDictionaryWriter::ingest_numeric_json_timestamp(std::string_view key,
                                                              int32_t node_id,
                                                              std::string_view timestamp)
        -> std::pair<epochtime_t, uint64_t> {
    auto& [_, timestamp_entry] = *m_column_id_to_range.try_emplace(node_id, key, node_id).first;
    ...
    auto const optional_parsed_timestamp{
            timestamp_parser::search_known_timestamp_patterns(timestamp,
    ...
    auto const new_pattern_id{m_next_id++};
    m_numeric_pattern_to_id.emplace(std::string{pattern},
                                    std::make_pair(std::move(pattern_result.value()),
                                                   new_pattern_id));
    return {epoch_timestamp, new_pattern_id};
}

auto TimestampDictionaryWriter::ingest_unknown_precision_epoch_timestamp(std::string_view key,
                                                                         int32_t node_id,
                                                                         int64_t timestamp)
        -> std::pair<epochtime_t, uint64_t> {
    auto& [_, timestamp_entry] = *m_column_id_to_range.try_emplace(node_id, key, node_id).first;
    ...
    if (m_numeric_pattern_to_id.end() == pattern_it) {
        ...
        auto const new_pattern_id{m_next_id++};
        pattern_it = m_numeric_pattern_to_id
                             .emplace(std::move(pattern),
                                      std::make_pair(std::move(pattern_result.value()),
                                                     new_pattern_id))
                             .first;
    }
