...
void MySQLIndexStorage::init(std::string const& dataset_name, bool should_create_table) {
    ...
    if (should_create_table) {
        m_db.execute_query(fmt::format("CREATE TABLE IF NOT EXISTS {} ("
...
void MySQLIndexStorage::add_field(std::string const& field_name, NodeType field_type) {
    ...
    statement_bindings.bind_varchar(clp::enum_to_underlying_type(TableMetadataFieldIndexes::Name),
                                    field_name.c_str(),
                                    field_name.length());

    auto field_type_value = static_cast<uint8_t>(field_type);
    statement_bindings.bind_uint8(clp::enum_to_underlying_type(TableMetadataFieldIndexes::Type),
                                  field_type_value);
