...
auto insert_invalid_node_id_value_pairs_with_node_type_errors(
        ...
        KeyValuePairLogEvent::NodeIdValuePairs& invalid_node_id_value_pairs) -> void;
...
[[nodiscard]] auto assert_kv_pair_log_event_creation_failure(
        ...
        std::errc expected_error_code) -> bool;
...
auto test_value_get_immutable_view(Value const& value, Type const& typed_value) -> void {
    ...
    if constexpr (std::is_same_v<EightByteEncodedTextAst, Type>) {
        REQUIRE((value.get_immutable_view<Type>() == typed_value));
        REQUIRE((std::is_same_v<EightByteEncodedTextAst const&,
    ...
    if constexpr (std::is_same_v<FourByteEncodedTextAst, Type>) {
        REQUIRE((value.get_immutable_view<Type>() == typed_value));
        REQUIRE((std::is_same_v<FourByteEncodedTextAst const&,
                                decltype(value.get_immutable_view<Type>())>));
    } else {
...
auto assert_kv_pair_log_event_creation_failure(
        ...
        UtcOffset utc_offset,
        std::errc expected_error_code) -> bool {
    auto const result{KeyValuePairLogEvent::create(std::move(auto_gen_keys_schema_tree),
...
TEST_CASE("ffi_KeyValuePairLogEvent_create", "[ffi]") {
    ...
    SECTION("Test empty ID-value pairs") {
        auto const result{KeyValuePairLogEvent::create(auto_gen_keys_schema_tree,
    ...
    SECTION("Test schema tree pointers being null") {
        REQUIRE(assert_kv_pair_log_event_creation_failure(nullptr,
                                                          user_gen_keys_schema_tree,
                                                          {},
                                                          {},
                                                          UtcOffset{0},
                                                          std::errc::invalid_argument));
        REQUIRE(assert_kv_pair_log_event_creation_failure(auto_gen_keys_schema_tree,
                                                          nullptr,
                                                          {},
                                                          {},
                                                          UtcOffset{0},
    ...
    SECTION("Test mismatched types") {
        ...
        // Int:
        insert_invalid_node_id_value_pairs_with_node_type_errors(*user_gen_keys_schema_tree,
                                                                 2,
    ...
    SECTION("Test valid ID-value pairs") {
        ...
        SECTION("Test JSON serialization") {
            ...
            auto const& [serialized_auto_gen_kv_pairs,
                         serialized_user_gen_kv_pairs]{serialized_json_result.value()};
