...
void literallyCompare(std::filesystem::path const& expected_output_json_path,
...
void check_all_leaf_nodes_match_types(std::set<clp_s::NodeType> const& types) {
    ...
    for (auto const& entry : std::filesystem::directory_iterator(cTestEndToEndArchiveDirectory)) {
        REQUIRE_NOTHROW(archive_reader.open(clp_s::Path{.source = clp_s::InputSource::Filesystem,
...
void validate_archive_header() {
    ...
    for (auto const& entry : std::filesystem::directory_iterator(cTestEndToEndArchiveDirectory)) {
        ...
        constexpr auto cReservedPaddingLength{sizeof(archive_header.reserved_padding)
                                              / sizeof(archive_header.reserved_padding[0])};
...
auto extract() -> std::filesystem::path {
    ...
    for (auto const& entry : std::filesystem::directory_iterator(cTestEndToEndArchiveDirectory)) {
        constructor_option.archive_path = clp_s::Path{.source{clp_s::InputSource::Filesystem},
                                                      .path{entry.path().string()}};
...
void compare(std::filesystem::path const& extracted_json_path) {
    ...
    REQUIRE((0 == result));
    auto command = fmt::format("jq --sort-keys --compact-output '.' {} | sort > {}",
    ...
    REQUIRE((0 == result));
    command = fmt::format("diff --unified {} {}  > /dev/null",
                          cTestEndToEndOutputSortedJson,
...
void literallyCompare(std::filesystem::path const& expected_output_json_path,
                      std::filesystem::path const& extracted_json_path) {
    auto command = fmt::format("cat {} | awk 'NF' | tr -d ' ' | sort > {}",
    ...
    command = fmt::format("cat {} | awk 'NF' | tr -d ' ' | sort > {}",
                          extracted_json_path.string(),
                          cTestEndToEndOutputSortedJson);
    result = std::system(command.c_str());
    REQUIRE((0 == result));
...
TEST_CASE("clp-s-compress-extract-no-floats", "[clp-s][end-to-end]") {
    ...
    TestOutputCleaner const test_cleanup{{std::string{cTestEndToEndArchiveDirectory},
                                          std::string{cTestEndToEndOutputDirectory},
                                          std::string{cTestEndToEndOutputSortedJson}}};

    REQUIRE_NOTHROW(std::ignore
                    = compress_archive(get_test_input_local_path(cTestEndToEndInputFile),
...
TEST_CASE("clp-s-compress-extract-valid-formatted-floats", "[clp-s][end-to-end]") {
    ...
    REQUIRE_NOTHROW(std::ignore = compress_archive(
    ...
    auto const expected_matching_types{
            structurize_arrays ? std::set<clp_s::NodeType>{clp_s::NodeType::FormattedFloat}
                               : std::set<clp_s::NodeType>{clp_s::NodeType::FormattedFloat,
                                                           clp_s::NodeType::UnstructuredArray}};
    check_all_leaf_nodes_match_types(expected_matching_types);

    auto extracted_json_path = extract();
    literallyCompare(get_test_input_local_path(cTestEndToEndValidFormattedFloatInputFile),
                     extracted_json_path);
}
...
TEST_CASE("clp-s-compress-extract-invalid-formatted-floats", "[clp-s][end-to-end]") {
    ...
    auto const expected_matching_types{
            structurize_arrays ? std::set<clp_s::NodeType>{clp_s::NodeType::DictionaryFloat}
                               : std::set<clp_s::NodeType>{clp_s::NodeType::DictionaryFloat,
                                                           clp_s::NodeType::UnstructuredArray}};
    check_all_leaf_nodes_match_types(expected_matching_types);

    auto extracted_json_path = extract();
    literallyCompare(get_test_input_local_path(cTestEndToEndInvalidFormattedFloatInputFile),
                     extracted_json_path);
}
...
TEST_CASE("clp-s-compress-extract-timestamps", "[clp-s][end-to-end]") {
    ...
    REQUIRE_NOTHROW(std::ignore
                    = compress_archive(get_test_input_local_path(cTestEndToEndTimestampInputFile),
                                       std::string{cTestEndToEndArchiveDirectory},
    ...
    auto extracted_json_path = extract();
    literallyCompare(get_test_input_local_path(cTestEndToEndTimestampInputFile),
                     extracted_json_path);
}
